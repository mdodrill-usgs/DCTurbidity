// This model is based on "Model_Turb_v7.stan" (folder DCTurbidity/Stan_Code)
// and "Model_Ind_v1.stan" (folder: DiscreteChoice/Stan_Code)  
// This version combines the basic structure of the Model_Ind_v1.stan
// with the covariates in the turbidity version (...v7)

// Alternative structure for the taxa covars...
// Added alternative sturcture for the R^2 calc for the trip_site level
// & calc for the individual level
...
data {
  ...
  vector[Nst] fish_sz_st;  // mean fork length of fish for each trip and site
...
generated quantities {
  ...
  real vREall_FEall_ind;
  matrix[Nind, Nspsz] tmp_vREall_FEall_ind;
  real vREall_ind;
  ...
  // for the trip & site level variation
  real VRE_FE_st;
  matrix[Nst, Nspsz] tmp_VRE_FE_st;
  real VRE_st;
  ...
  for(k in 1:Nsp){
   ...
   for(i in 1:Nind){
    ...
    tmp_vREall_FEall_ind[i,k] = beta1[i,k] - spsz_eta[k];
  ...
  // for the individual level variation
  vREall_FEall_ind = variance(tmp_vREall_FEall_ind[]);
  vREall_ind = variance(spsz_ind_eta);
  
  // for the trip & site level variation
  for(i in 1:Nst){
    for(k in 1:Nspsz){
      tmp_VRE_FE_st[i,k] = dot_product(fix_beta_sp[i,], X[k,]) + 
                            dot_product(beta_sz[i,], X[k,]) * (log(sz[k]) - avg_log_len) +
                            beta_f_sz * fish_sz_st[i] * (log(sz[k]) - avg_log_len) +
                            spsz_st_eta[idx_ts_ind[i],k];
    }
  }
  
  VRE_st = variance(spsz_st_eta[]);
  VRE_FE_st = variance(tmp_VRE_FE_st[]);
